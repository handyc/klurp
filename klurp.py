...
print(f"Welcome to {friendlyname}, {user}!")
print(f"{friendlyname} start: {str(start_string)}")
...
print(f"{str(argument_count - 1)} arguments:", collapsed, " --->")
...
if argument_count > 6 and sys.argv[4].isnumeric():
    ...
    print("usage:")
    print(f"{shortname} [witness1] [witness2] [popsize] [generations] [mutation rate] [dictionary location]")
    print("")
    print("if population size is too low, evolution may not occur properly")
    print("")
    print("example:")
    print(f"python3 {shortname} T310.31 D75 100 100 7 /Users/handyc/data/bls/example1/dictionaries")
    exit()
...
if len(sortedglob1) > 0:
    ...
    print(f"Could not find a match for {globmatch1}, quitting")
    exit()
...
if len(sortedglob2) > 0:
    ...
    print(f"Could not find a match for {globmatch2}, quitting")
    exit()
...
mut = str(mutrate)
fullcommand = (
    f"{engine}population.py {gen} {popnum} {mut} {arg1} {arg2} {dictloc}"
)
...
print("")
print(f"{friendlyname} start: {str(start_string)} end: {str(end_string)}")
print(f"{friendlyname} total processing time: {str(total_time)}")
